...
function tryToBuyBestServerPossible(ns) {
  ...
  var exponentLevel = 1;
  while (
    Math.pow(2, exponentLevel + 1) * purchasedServerCostPerRam <=
      currentMoney &&
    exponentLevel < maxPurchasedServerRamExponent
  ) {
    exponentLevel += 1;
  }
  ...
  if (
    maxRamPossibleToBuy < ns.getServerRam("home")[0] &&
    maxRamPossibleToBuy < Math.pow(2, maxPurchasedServerRamExponent)
  ) {
    return "";
  }
  ...
  if (isWorseThanExistingServer) return "";
